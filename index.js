...
`;
//#endregion
//#region extensions/tdx-finance/src/wenda-tool.ts
...
const WENDA_NOTICE_TOOL_NAME = "wenda_notice_query";
const WENDA_REPORT_TOOL_NAME = "wenda_report_query";
const WENDA_NEWS_TOOL_NAME = "wenda_news_query";
const WENDA_MACRO_TOOL_NAME = "wenda_macro_query";
const WENDA_NOTICE_TOOL_LABEL = "公告查询";
const WENDA_REPORT_TOOL_LABEL = "研报查询";
const WENDA_NEWS_TOOL_LABEL = "资讯查询";
const WENDA_MACRO_TOOL_LABEL = "宏观数据查询";
const WendaToolSchema = Type.Object({
	query: Type.Optional(Type.String({ description: "Natural language query or legacy pipe query string" })),
	symbol: Type.Optional(Type.String({ description: "Optional symbol or subject name" })),
	name: Type.Optional(Type.String({ description: "Structured subject name for announcement/report/news/macro search" })),
	bdate: Type.Optional(Type.String({ description: "Begin date in YYYYMMDD" })),
	edate: Type.Optional(Type.String({ description: "End date in YYYYMMDD" })),
	keywords: Type.Optional(Type.String({ description: "Keywords separated by commas" })),
	desc: Type.Optional(Type.String({ description: "Additional raw description" })),
	raw: Type.Optional(Type.String({ description: "Raw query text forwarded as the raw parameter" })),
	top_k: Type.Optional(Type.Number({ description: "Reserved field, not forwarded unless backend supports it" })),
	time_range: Type.Optional(Type.String({ description: "Reserved field, not forwarded unless backend supports it" })),
	extra: Type.Optional(Type.Record(Type.String(), Type.Unknown()))
}, { additionalProperties: false });
const WENDA_TOOLS = [
	{
		name: WENDA_NOTICE_TOOL_NAME,
		label: WENDA_NOTICE_TOOL_LABEL,
		description: `
公告查询工具。
用于查询公司的公告、临时公告、定期报告等信息。优先使用自然语言 \`query\`，也支持结构化参数组合。

## 常用参数

| 参数 | 必填 | 说明 |
|------|------|------|
| query | 否 | 自然语言查询，或旧格式 \`主体|开始日期|结束日期|关键词|补充说明\` |
| symbol | 否 | 证券代码、简称或主体名称 |
| name | 否 | 主体名称；结构化查询时优先级高于 \`symbol\` |
| bdate | 否 | 开始日期，格式 \`YYYYMMDD\` |
| edate | 否 | 结束日期，格式 \`YYYYMMDD\` |
| keywords | 否 | 关键词，多个关键词用逗号分隔 |
| desc | 否 | 附加说明 |
| raw | 否 | 原样透传给下游接口的 \`raw\` 参数 |
| extra | 否 | 结构化扩展字段对象 |

## 使用示例

\`\`\`bash
wenda_notice_query query="查询宁德时代最近一个月的公告"
wenda_notice_query name="贵州茅台" bdate="20260101" edate="20260326" keywords="分红,董事会"
wenda_notice_query query="比亚迪|20260101|20260326|回购,定增|只看临时公告"
\`\`\`
`,
		apiUrlEnvNames: ["WENDA_NOTICE_API_URL", "ANNOUNCEMENT_API_URL"],
		defaultApiUrl: "https://www.tdx.com.cn/wenda/api/tools/gg_search"
	},
	{
		name: WENDA_REPORT_TOOL_NAME,
		label: WENDA_REPORT_TOOL_LABEL,
		description: `
研报查询工具。
用于查询券商研报、评级调整、目标价和观点摘要等信息。优先使用自然语言 \`query\`，也支持结构化参数组合。
...
\`\`\`bash
wenda_report_query query="查一下比亚迪最近两周的券商研报"
wenda_report_query name="中际旭创" bdate="20260301" edate="20260326" keywords="评级,目标价"
wenda_report_query query="查一下半导体行业最新深度研报"
\`\`\`
`,
		apiUrlEnvNames: ["WENDA_REPORT_API_URL", "REPORT_API_URL"],
		defaultApiUrl: "https://www.tdx.com.cn/wenda/api/tools/yb_query"
	},
	{
		name: WENDA_NEWS_TOOL_NAME,
		label: WENDA_NEWS_TOOL_LABEL,
		description: `
资讯查询工具。
用于查询新闻、快讯、主题资讯和公司相关资讯。优先使用自然语言 \`query\`，也支持结构化参数组合。
...
\`\`\`bash
wenda_news_query query="查一下寒武纪最近的新闻"
wenda_news_query name="中芯国际" bdate="20260301" edate="20260326" keywords="扩产,先进制程"
wenda_news_query query="查一下机器人板块今天的资讯"
\`\`\`
`,
		apiUrlEnvNames: ["WENDA_NEWS_API_URL", "NEWS_API_URL"],
		defaultApiUrl: "https://www.tdx.com.cn/wenda/api/tools/zx_search"
	},
	{
		name: WENDA_MACRO_TOOL_NAME,
		label: WENDA_MACRO_TOOL_LABEL,
		description: `
宏观数据查询工具。
用于查询宏观经济、产业景气、价格指数、利率、货币、社融等宏观数据。优先使用自然语言 \`query\`，也支持结构化参数组合。
...
|------|------|------|
| query | 否 | 自然语言查询，或旧格式 \`主体|开始日期|结束日期|关键词|补充说明\` |
| symbol | 否 | 指标代码、指标简称或主题名称 |
| name | 否 | 指标名称；结构化查询时优先级高于 \`symbol\` |
| bdate | 否 | 开始日期，格式 \`YYYYMMDD\` |
| edate | 否 | 结束日期，格式 \`YYYYMMDD\` |
| keywords | 否 | 关键词，多个关键词用逗号分隔 |
| desc | 否 | 附加说明 |
...
\`\`\`bash
wenda_macro_query query="查询最近一年的CPI和PPI走势"
wenda_macro_query name="社会融资规模" bdate="20250101" edate="20260326" keywords="同比,新增"
wenda_macro_query query="查询美国10年期国债收益率最近三个月数据"
\`\`\`
`,
		apiUrlEnvNames: [
			"WENDA_MACRO_API_URL",
		...
		defaultApiUrl: "https://www.tdx.com.cn/wenda/api/tools/edb_query"
	}
];
...
function readEnv(name) {
	try {
		return (typeof process !== "undefined" ? process.env?.[name] : void 0)?.trim() || void 0;
	} catch {
		return;
	}
}
...
function asOptionalString(value) {
	return isNonBlank(value) ? value.trim() : void 0;
}
function normalizeAuthToken(value) {
	const text = asOptionalString(value);
	if (!text) return;
	return text.match(/^tdx-auth\s+(.+)$/i)?.[1]?.trim() || text;
}
...
function buildStructuredQuery(params) {
	const extra = params.extra ?? {};
	return [
		firstNonBlank(params.name, extra.name, params.symbol, extra.symbol, params.query) ?? "",
		normalizeDate(firstNonBlank(params.bdate, extra.bdate)),
		normalizeDate(firstNonBlank(params.edate, extra.edate)),
		firstNonBlank(params.keywords, extra.keywords) ?? "",
		firstNonBlank(params.raw, extra.raw, params.desc, extra.desc) ?? ""
	].join("|");
}
function buildSearchQuery(params) {
	...
	if ([
		...
		extra.desc,
		extra.raw
	].some(isNonBlank)) return buildStructuredQuery(params);
	return firstNonBlank(params.query) ?? "";
}
function buildLegacyHttpParams(query) {
	const parts = query.split("|", 5);
	const target = (parts[0] ?? "").replace(/:/g, "");
	const bdate = normalizeDate(parts[1]?.trim());
	const edate = normalizeDate(parts[2]?.trim());
	const keywords = (parts[3] ?? "").split(/[、，]/).map((part) => part.trim()).filter(Boolean).join(" ");
	const raw = asOptionalString(parts[4]);
	return {
		query: `${target}:${bdate}-${edate}:${keywords}`,
		raw
	};
}
function resolveApiUrl(params, context) {
	if (isNonBlank(context.apiUrl)) return context.apiUrl.trim();
	for (const envName of params.apiUrlEnvNames) {
		const value = readEnv(envName);
		if (value) return value;
	}
	return isNonBlank(params.defaultApiUrl) ? params.defaultApiUrl.trim() : void 0;
}
...
function requiresTdxAuth(apiUrl) {
	try {
		const url = new URL(apiUrl);
		return url.protocol === "https:" && url.hostname === "www.tdx.com.cn" && url.pathname.startsWith("/wenda/api/tools/");
	} catch {
		return false;
	}
}
function usesPipeDelimitedQuery(apiUrl) {
	return requiresTdxAuth(apiUrl);
}
async function fetchText(params) {
	const controller = new AbortController();
	const timeout = setTimeout(() => controller.abort(), params.timeoutMs);
	const abortHandler = () => controller.abort();
	if (params.signal) params.signal.addEventListener("abort", abortHandler);
	try {
...
function createWendaTool(params) {
	const logger = params.context.logger;
	const timeoutMs = params.context.timeoutMs || readEnvInt("WENDA_TIMEOUT_MS") || DEFAULT_TIMEOUT_MS;
	return {
		name: params.name,
		label: params.label,
		description: params.description,
		parameters: WendaToolSchema,
		async execute(_toolCallId, toolParams, signal) {
			const normalizedQuery = buildSearchQuery(toolParams);
			if (!normalizedQuery) return json({
				ok: false,
				tool: params.label,
				tool_name: params.name,
				error: "query or structured fields cannot be empty"
			});
			const apiUrl = resolveApiUrl(params, params.context);
			if (!apiUrl) return json({
				ok: false,
				tool: params.label,
				tool_name: params.name,
				query: normalizedQuery,
				error: "api url is not configured",
				hint: `Please set one of: ${params.apiUrlEnvNames.join(", ")}`
			});
			const authToken = resolveAuthToken(params.context);
			if (requiresTdxAuth(apiUrl) && !authToken) return json({
				ok: false,
				tool: params.label,
				tool_name: params.name,
				query: normalizedQuery,
				error: "tdx auth token is not configured",
				hint: `Please set one of: ${WENDA_AUTH_ENV_NAMES.join(", ")}`
			});
			const startedAt = Date.now();
			try {
				const url = new URL(apiUrl);
				if (usesPipeDelimitedQuery(apiUrl)) url.searchParams.set("query", normalizedQuery);
				else {
					const legacyParams = buildLegacyHttpParams(normalizedQuery);
					url.searchParams.set("query", legacyParams.query);
					if (legacyParams.raw) url.searchParams.set("raw", legacyParams.raw);
				}
				logger?.debug?.(`tdx-finance/wenda: fetch ${url.toString()}`);
				const responseText = await fetchText({
					url: url.toString(),
					timeoutMs,
					headers: appendTdxTokenHeader(authToken ? { "Tdx-Auth": authToken } : {}, params.context.tdxApiToken),
					signal
				});
				return json({
					ok: true,
					...
					data: parseMaybeJson(responseText)
				});
			} catch (error) {
				const message = error instanceof Error ? error.message : String(error);
				logger?.error(`tdx-finance/wenda: ${params.name} (${params.label}) failed - ${message}`);
				return json({
					ok: false,
					tool: params.label,
					tool_name: params.name,
					query: normalizedQuery,
					error: message,
					hint: `Please ensure the HTTP endpoint is reachable via one of: ${params.apiUrlEnvNames.join(", ")}`,
					elapsed_ms: Date.now() - startedAt
				});
			}
		}
...
const plugin = {
	...
	register(api) {
		...
		for (const tool of WENDA_TOOLS) {
			api.registerTool(createWendaTool({
				...tool,
				context
			}));
			api.logger.info(`tdx-finance: registered tool: ${tool.name}`);
